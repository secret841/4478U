...
motor RightFrontMotor = motor(PORT20, ratio6_1, false);
...
void drivePID (double desiredPosition, int velocit)
{
  ...
  while (error > 5)
  {
    ...
    double encoderValue = (LeftFrontMotor.position(degrees) + LeftMiddleMotor.position(degrees) +
    LeftBackMotor.position(degrees) + RightFrontMotor.position(degrees) + RightMiddleMotor.position(degrees) +
    RightBackMotor.position(degrees)) / 6; 
...
void usercontrol(void) {
  ...
  if (fabs(LSpeed) >= 10 || fabs(RSpeed) >= 10)
  {
    ...
    LeftMiddleMotor.spin(forward, LSpeed, percent);
    RightBackMotor.spin(forward, RSpeed, percent);
    RightMiddleMotor.spin(forward, RSpeed, percent);
  ...
  {
    Intake.spin(forward, 88, percent);
  }
  else if (Controller1.ButtonL2.pressing())
  {
    Intake.spin(reverse, 88, percent);
  }
  else
  {
